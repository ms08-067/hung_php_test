...
class AppRepository
{
	...
	public function sendMail($to, $subject, $body, $cc = [], $bcc = [], $attach_file = [],$from = 'info@emoseeker.com', $from_name = 'emoseeker.com', $to_name=''){
	...
	public function userMarketingConfirm( \Nova\User\Models\UserMarketing $subcribe)
	{
		ini_set('max_execution_time', 600);
		$template = \Nova\Common\Models\EmailTemplate::where('slug','email_user_marketing_confirm')->first();
		
		if(!empty($template)){
			
			$display_name = empty($subcribe->l_name) ? substr($subcribe->email, 0, strpos($subcribe->email, '@')): $subcribe->l_name;

			$data = array(
				"display_name" => "<strong>".ucwords($display_name)."</strong>",
				"user_marketing_confirm"  => route('subscribe.confirm')."?key=".$subcribe->active_key
			);

			$content = $this->parserString($template->content, $data);

			try{
				Mail::send([], [], function($message) use ($template, $subcribe, $content)
				{
					$message->to($subcribe->email, $subcribe->f_name." ".$subcribe->l_name);
	...
	public function get_list_grammar(){
		return \Nova\Admin\Models\Content::where("public_flag",1)->get();
	}
